...
model_name=$4
schedule=$5 #"fast" or "full"
noisy_place=$6 # "none" or "noisy_in" or "noisy_out" or "noisy_inout"
...
noisy_command="--${noisy_place}"

if [[ "$noisy_place" == "none" ]]; then
    noisy_command=""
fi
...
if [ ${stage} -le 2 ]; then
    ...
    enhanced_path=${diffwave}/Enhanced/${model_name}/model${ckp}_${schedule}_${noisy_place}/test
    rm -r ${enhanced_path} 2>/dev/null
    mkdir -p ${enhanced_path} 
    echo "inference enhanced wav file from ${spec_root} to ${enhanced_path}"
    
    python src/diffwave/inference_noisy.py  ${diffwave}/${model_name}/weights-${ckp}.pt ${test_spec_list} ${voicebank_noisy} -o ${enhanced_path}  --${task} --voicebank --${schedule} ${noisy_command}
fi
if [ ${stage} -le 3 ]; then
    echo "stage 3 : scoring"
    
    score_file=${diffwave}/Enhanced/${model_name}/scores_test.csv
    clean_wav=${voicebank_clean}
    enhanced_result=${diffwave}/Enhanced/${model_name}/model${ckp}_${schedule}_${noisy_place}/test
    echo "save the score to ${score_file}"
    cd pesq/speech-metrics/
    python main.py ${clean_wav} ${enhanced_result} ${score_file} model${ckp}_${schedule}_${noisy_place}
    cd -
fi
